      ...
      <div className="lg:hidden">
        <Select value={activeId ?? undefined} onValueChange={onSelect}>
          <SelectTrigger className="w-full">
            <span className="text-label-sm font-semibold text-muted mr-2">PILIH MODUL</span>
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            {experiments.map((exp) => {
              ...
              return (
                <SelectItem key={exp.id} value={exp.id} disabled={isLocked}>
                  <div className="flex flex-col">
                    <span>{exp.id}</span>
                    <span className="text-label-sm text-muted">{exp.name}</span>
                  </div>
                </SelectItem>
              )
            })}
